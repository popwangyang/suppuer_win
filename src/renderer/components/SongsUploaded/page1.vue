<template>
	<div id="SongsUploaded" :class="{ff:flage,gg:!flage}">
        ...
        <i class="el-icon-upload" style="font-size: 20px; cursor: pointer; margin: 0px 5px;" @click="uploadFile(scope.row.id)" v-if="scope.row.content.status == 0"></i>
				<i class="el-icon-upload" style="font-size: 20px; cursor: pointer; margin: 0px 5px;cursor: not-allowed;color: #d4d4d4;" v-if="scope.row.content.status == 1"></i>
        <i class="el-icon-upload" style="font-size: 20px; cursor: pointer; margin: 0px 5px;cursor: not-allowed;color: #d4d4d4;" v-if="scope.row.content.status == 2"></i>
				<i class="el-icon-upload" style="font-size: 20px; cursor: pointer; margin: 0px 5px;" @click="uploadFile(scope.row.id)" v-if="scope.row.content.status == 3"></i>
	...
	export default{
      ...
      this.$store.commit("uploadFile",id)
			...
			JumpPage(id) {
				...
				this.$router.push({path:"/editPage",query:{fileID:id,"page":"SU"}})
			},
			deleteFile(id){
...
// 						})
	  ...
	  asyncTesk(data,id){
			var _this = this;
			get("/music/music/task-result",{"task_id":id}).then(function(res1){
						console.log(res1)
