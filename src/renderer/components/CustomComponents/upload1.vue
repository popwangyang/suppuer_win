		...
		methods: {
			...
			send_data(result, file, currentNum) {
				...
				if (file.content.status == 3) {
					...
					console.log("替换")
					var name = obj.singer + "￥" + obj.name + "￥" + obj.singer_type + "￥" + obj.language + "￥" + obj.picture + "￥" +
						obj.area + "￥" + obj.voice_type + "￥" + obj.format_type + "￥" + obj.voice_track + "￥" + obj.vocal_track + "." +
						file.type;
					fd.append("opera", 1);
				...
				xhr.onreadystatechange = function() {
					if (xhr.readyState == 4 && xhr.status == 200) {
						...
						var restNumber = Math.floor((file.size - currentNum) / (1024 * 1024 * speed) < 0 ? 0 : ((file.size - currentNum) /
							(1024 * 1024 * speed)))
