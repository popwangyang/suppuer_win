<template>
	<div id="Main" style="-webkit-user-select:none;overflow: hidden;" @click="goo">
					...
					<div style="width: 100%;height: 30px;display: flex;flex-direction: row-reverse;">
						...
						<div style="flex: 1;display: flex;">
							<div class="headBtn" style="display: flex;">
								...
								<b style="display: block;margin: 3px 10px ;background:#a8a0fb;height: 24px;width: 1px;margin-right: 30px;"
								 v-show="shugangFlage"></b>
								<!--
				...
				<el-aside width="200px">
					<el-row>
						<el-col :span="24">
							...
							<el-menu active-text-color="#333333" class="el-menu-vertical-demo" @select='ItemSelect'>
								...
								<el-menu-item index="CustomManagement">
									...
									<span v-if="DZnumber>0">定制管理
										<el-badge class="mark" :value="DZnumber" :max="99" style="margin-bottom: 6px;" /></span>
								...
								<el-menu-item index="SongsUploaded">
									...
									<span v-if="DRnumber>0">歌曲导入
										<el-badge class="mark" :value="DRnumber" :max="99" style="margin-bottom: 6px;" /></span>
									<span v-else>歌曲导入 <span style="display: inline-block;height: 100%;width: 21px;"></span> </span>
								</el-menu-item>
								<el-menu-item index="Uploading">
									<i class="el-icon-upload"></i>

									<span slot="title" v-if="UPnumber>0">正在上传
										<el-badge class="mark" :value="UPnumber" :max="99" style="margin-bottom: 6px;" /></span>
		...
		<!--忘记密码-->
		<el-dialog title="修改密码" :visible.sync="changePasswordFlage" @close="resetForm('ruleForm2')" custom-class="DELECT"
		 width="30%" top='25vh'>
			<el-form :label-position="labelPosition" label-width="80px" :model="ruleForm2" size="mini" :rules="rules2" ref="ruleForm2">
		...
		<el-dialog title="提示" :visible.sync="dialogUploadVisible" width="30%" top='25vh'>
		...
		<el-dialog title="关闭提示" :visible.sync="dialogcloseSettingVisible" width="25%" custom-class="closeSetting" top='25vh'>
		...
		<el-dialog title="提示" :visible.sync="dialogExitLoginVisible" custom-class="PLSOSU" width="30%" top='25vh'>
		...
		<el-dialog title="提示" :visible.sync="dialogcloseVisible" custom-class="PLSOSU" width="30%" top='25vh'>
		...
		<el-dialog title="更新提示" :visible.sync="dialogQZupdateVisible" custom-class="PLSOSU" :close-on-click-modal="false"
		 :close-on-press-escape="false" width="30%" top='25vh'>
		...
		<el-dialog title="更新提示" :visible.sync="dialogHDupdateVisible" custom-class="PLSOSU" width="30%" top='25vh'>
		...
		<el-dialog title="更新提示" :visible.sync="dialogHD_notupdateVisible" custom-class="PLSOSU" width="30%" top='25vh'>
		...
		<el-dialog title="更新提示" :visible.sync="dialogHD_errorupdateVisible" custom-class="PLSOSU" width="30%" top='25vh'>
		...
		<el-dialog title="更新提示" :visible.sync="dialogHDupdateProgressVisible" custom-class="PLSOSU" width="30%" top='25vh'>
		...
		<el-dialog title="安装提示" :visible.sync="dialogupdateInstallVisible" custom-class="PLSOSU" width="30%" top='25vh'>
	...
	export default {
		...
		methods: {
			...
			exitLogin() {
				...
				if (this.childWindowFlage) {
					if (this.$store.state.SongUpload.uploadSongNumbers > 0) {
		...
		mounted() {
			...
			readStoreDB(this, {
				...
				this.$store.commit('setUplaodNum', res.length);
			})
			var _this = this;
			setTimeout(() => {
				this.updateStep = 1;
...
/* .slide-fade-leave-active for below version 2.1.8 */
	...
	#Main .headBtn .headBtn_back_left,
	#Main .headBtn .headBtn_refresh,
	#Main .headBtn .headBtn_back_right {
		border: 1px solid white;
	}

	#Main .headBtn .headBtn_back_left1,
	#Main .headBtn .headBtn_back_right1 {
		border: 1px solid #b6bcce;
		cursor: not-allowed;

	}

	#Main .headBtn .headBtn_back_left1 .el-icon-arrow-left1,
	#Main .headBtn .headBtn_back_right1 .el-icon-arrow-left1 {
		font-size: 20px;
		color: #b6bcce;
	}
	...
	#Main .headBtn .headBtn_back_left .el-icon-arrow-left1,
	#Main .headBtn .headBtn_back_right .el-icon-arrow-left1 {
		font-size: 20px;
		color: white;
