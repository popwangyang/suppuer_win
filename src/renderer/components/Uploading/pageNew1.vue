<template>
	<div id="Uploading" :class="{ff:boxFlage,gg:!boxFlage}">
		...
		<div class="box2" v-show="!boxFlage">
			<div class="box2a" :style="height" id="scroll-1">
				<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
					...
					<el-table-column prop="content.singer" label="歌手名称" min-width="60">
					</el-table-column>
					...
					<el-table-column prop="content.format_type" label="格式" min-width="60">
						<template slot-scope="scope">
							...
							<span v-else-if="scope.row.content.format_type =='4' || scope.row.content.format_type =='MKV'">
								MKV
							</span>
					...
					<el-table-column prop="upState" label="状态" min-width="120">
						<template slot-scope="scope">
							...
							<div v-if="scope.row.upState=='7'">
								<i style="color: red;">歌曲信息重复</i>
								<el-button type="text" style="font-size: 12px;margin-left: 10px;" @click="moveRepeat(scope.row)">请选择操作</el-button>
							</div>
						...
						<template slot-scope="scope">
							<div style="display: flex;justify-content: center;">
								<div style="width: 24px;height: 24px;display: flex;justify-content: center;align-items: center;margin: 0px 10px;">
									...
									<i class="iconfont icon-xiazaizanting" style="font-size: 20px; cursor: pointer;" v-if="scope.row.upState=='1'"
									 @click="uploadBtn(scope.row.id)"></i>
		...
		<el-dialog title="提示" @close="beforeClose" :visible.sync="delecteVisible" custom-class="DELECT" width="30%" top='30vh'>
		...
		<!--//重复操作框-->
		<el-dialog title="提示" :visible.sync="moveRepeatVisible" width="84%">
			...
			<span style="border: 1px solid #ebeef5;display: block;position: relative;">
				<el-table :data="tableData2" :row-class-name="tableRowClassName" max-height="300" style="width: 100%"
		...
		methods: {
			...
			tihuan(id) {
				...
				if (this.$store.getters.number < 3) {
					this.$store.commit("startUplaodRepeat", {
						fileID: this.moveRepeatID,
						THid: id
					})

				} else {
					this.$store.commit("startUplaodRepeat1", {
						fileID: this.moveRepeatID,
						THid: id
					})
				}
			...
			xinZheng(id) {
				...
				if (this.$store.getters.number < 3) {
					...
					this.$store.commit("startUplaodxiZheng1", {
						fileID: this.moveRepeatID
					})
				}
			},
