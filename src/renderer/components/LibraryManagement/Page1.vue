<template>
	...
	<el-upload ref="upload"	v-show="showUpload" 
	:headers="headers" 
	class="upload-demo" 
	drag 
	...
	<!--替换弹框-->
	<el-dialog :title="replacefileName" @close="replaceClose" :visible.sync="replaceVisible" width="401px"  :close-on-click-modal="false" top='26vh' custom-class="PLSOSU">
		...
		<el-dropdown-item><el-radio v-model="radioForm.singer_type" :label="3"  @change="singer_typeFilter(3)">合唱</el-radio></el-dropdown-item>
			...
			<li :style="widthHead1[4]"  >
				<el-dropdown trigger="click" size="mini">
					...
					<el-dropdown-menu slot="dropdown" >
						<el-dropdown-item><el-radio v-model="radioForm.language" :label="100"  @change="languageFilter(100)">全部</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.language" :label="0"  @change="languageFilter(0)">国语</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.language" :label="1"  @change="languageFilter(1)">粤语</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.language" :label="2"  @change="languageFilter(2)">闽南语</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.language" :label="3"  @change="languageFilter(3)">英语</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.language" :label="4"  @change="languageFilter(4)">日语</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.language" :label="5"  @change="languageFilter(5)">韩语</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.language" :label="6"  @change="languageFilter(6)">其他</el-radio></el-dropdown-item>
					</el-dropdown-menu>
				</el-dropdown>
				</li>
			...
			<el-dropdown trigger="click" size="mini">
				...
				<el-dropdown-menu slot="dropdown" >
					<el-dropdown-item><el-radio v-model="radioForm.picture" :label="100"@change="pictureFilter(100)">全部</el-radio></el-dropdown-item>
					<!--<el-dropdown-item><el-radio v-model="radioForm.picture" :label="0"  @change="pictureFilter(0)">原版MV</el-radio></el-dropdown-item>-->
					<el-dropdown-item><el-radio v-model="radioForm.picture" :label="4"  @change="pictureFilter(4)">MTV</el-radio></el-dropdown-item>

					<el-dropdown-item><el-radio v-model="radioForm.picture" :label="1"  @change="pictureFilter(1)">LIVE</el-radio></el-dropdown-item>
					<el-dropdown-item><el-radio v-model="radioForm.picture" :label="2"  @change="pictureFilter(2)">舞曲</el-radio></el-dropdown-item>
					<el-dropdown-item><el-radio v-model="radioForm.picture" :label="3"  @change="pictureFilter(3)">配置画面</el-radio></el-dropdown-item>
			...
			<li :style="widthHead1[6]"  >
				...
				<el-dropdown trigger="click" size="mini">
					...
					<el-dropdown-menu slot="dropdown" >
						<el-dropdown-item><el-radio v-model="radioForm.area" :label="100"@change="areaFilter(100)">全部</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.area" :label="0"  @change="areaFilter(0)">大陆</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.area" :label="1"  @change="areaFilter(1)">港台</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.area" :label="2"  @change="areaFilter(2)">日韩</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.area" :label="3"  @change="areaFilter(3)">欧美</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.area" :label="4"  @change="areaFilter(4)">其他</el-radio></el-dropdown-item>
			...
			<li :style="widthHead1[7]"  >
				...
				<el-dropdown trigger="click" size="mini">
					...
					<el-dropdown-menu slot="dropdown" >
						<el-dropdown-item><el-radio v-model="radioForm.format_type" :label="100"@change="formatFilter(100)">全部</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.format_type" :label="0"  @change="formatFilter(0)">高清</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.format_type" :label="1"  @change="formatFilter(1)">DVD</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.format_type" :label="2"  @change="formatFilter(2)">MP4</el-radio></el-dropdown-item>
						<!-- <el-dropdown-item><el-radio v-model="radioForm.format_type" :label="3"  @change="formatFilter(3)">其他</el-radio></el-dropdown-item>			 -->
					</el-dropdown-menu>
				</el-dropdown>
				</li>	
			...
			>
				...
				<el-dropdown trigger="click" size="mini">
					...
					<el-dropdown-menu slot="dropdown" >
						<el-dropdown-item><el-radio v-model="radioForm.voice" :label="100"@change="voice_typeFilter(100)">全部</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.voice" :label="0"  @change="voice_typeFilter(0)">消音</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.voice" :label="1"  @change="voice_typeFilter(1)">原版伴奏</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.voice" :label="2"  @change="voice_typeFilter(2)">演唱会</el-radio></el-dropdown-item>
						<el-dropdown-item><el-radio v-model="radioForm.voice" :label="3"  @change="voice_typeFilter(3)">remix</el-radio></el-dropdown-item>	
						<el-dropdown-item><el-radio v-model="radioForm.voice" :label="4"  @change="voice_typeFilter(4)">其他</el-radio></el-dropdown-item>	
					</el-dropdown-menu>
				</el-dropdown>
				</li>			
          ...
          z-index: 2000000;					
				...
				id="ulo">
				  <li style="list-style: none;text-align: left;margin: 5px 0;font-size: 12px;"><el-radio v-model="radioForm.album_name" :label="100"  @change="album_nameFilter(100)"><span style="font-size: 12px;">全部</span></el-radio></li>  
					<li style="list-style: none;text-align: left;margin: 5px 0;font-size: 12px;" v-for="item in album_nameArr"><el-radio v-model="radioForm.album_name" :label="item.value" @change="album_nameFilter()"><span style="font-size: 12px;">{{item.value}}</span></el-radio></li>  
				</ul>
				</span>
			...
			<li :style="widthHead1[12]" style="position: relative;">
				...
				box-shadow: 0 2px 12px 0 rgba(0,0,0,.1);" 
				id="ulo">
					<li style="list-style: none;text-align: left;margin: 5px 0;font-size: 12px;"><el-radio v-model="radioForm.company_name" :label="100"  @change="company_nameFilter(100)"><span style="font-size: 12px;">全部</span></el-radio></li>  
					<li style="list-style: none;text-align: left;margin: 5px 0;font-size: 12px;" v-for="item in company_nameArr"><el-radio v-model="radioForm.company_name" :label="item.value" @change="company_nameFilter()"><span style="font-size: 12px;">{{item.value}}</span></el-radio></li>  
				</ul>
				</span>
				</span>
				</li>			
			...
			>
				...
				<el-table-column label="操作" fixed="right" :width="widthHead[14]">
					<template slot-scope="scope">
						...
						<i class="iconfont icon-bianji" style="font-size: 20px;cursor: pointer;margin: 0 5px;"  @click="JumpPage(scope.row.id)" title="编辑"></i>
						<i class="iconfont icon-dianliangqiang_daohang_tihuan_moren" title="替换" style="font-size: 20px;cursor: pointer;margin: 0 5px;"  @click="replaceFile(scope.row)"></i>
						<i class="iconfont icon-shanchu" style="font-size: 20px;cursor: pointer;margin: 0 5px;"  @click="deleteFile(scope.row.id)" title="删除"></i>
						</span>
					</template>
				</el-table-column>
			</el-table>
...
<script>
	...
	import { get , baseUrl , DELETE , post } from '../api'
	export default {
		components: { Loading, replaceFile },
		data() {
			return {
		...
		computed: {
			...
			TipsFlage:function(){
				...
				arr.map(function(item){
					//console.log(item.singer_type)
					  if(item.name == null || item.singer == null || item.singer_type == 4   || item.language == null ||item.picture == null || item.picture == 5  ||item.area == null ||item.format_type == null || item.format_type == 3  ||item.voice_type == null || item.voice_track == null ||  item.voice_track == 4 ||item.vocal_track == null || item.vocal_track == 4 ||item.update_date == null){
							n++;
							arrError.push(item)
						}else{
							arr1.push(item)
				...
				// this.tableData3 = arrError.concat(arr1)
				this.errorNumber = n;
				if(n>0){
					return true;
				}else{
		...
		loadAllCompany(){
			...
			getWidthHead(){
				...
				//console.log(this.widthHead1,this.$refs.box_head.getElementsByTagName("li"))	
			},
         ...
         //  单个删除文件         
           ...
           this.delecteVisible = true
            ...
            this.delecteLoading = true   
						////console.log(ID)
               // DELETE('/music/music/store/'+ID+'').then(function(res){
								 var send_data = {
									  ids:[ID] ,
										type: 4
								 }
...
//                 }
			...
			detail(id){		
				...
				if(id){
					...
					get('/music/music/store',send_data).then(function(response){
						_this.loading = false
						////console.log("ppooo",response.data.count)
						_this.count = response.data.count
						_this.tableData3 = response.data.results
						////console.log(response.data.results);//请求正确时执行的代码
		...
		mounted() {
		  this.user_id = localStorage.getItem("user_id");
			...
			this.loadAllCompany();
			// this.TipsFlage = true;
			//console.log(this.TipsFlage)
      ////console.log(this.headers,this.actionUrl)
			this.height = "height:" + (document.body.clientHeight - 239) + "px"
			// //console.log(this.$route.query,"ppppppppppppp")
			 
        // this.detail(this.$route.query.fileID)
			this.detail();
			
			// this.loadAll()
			// document.querySelector('#LibraryManagement')
// 			document.querySelector('#LibraryManagement').onresize = function() {
