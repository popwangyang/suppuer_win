...
<div class="box" ref="scroll_1"  :style="height" id="scroll-1" v-show="loading">
			...
			<template slot-scope="scope">
				...
				<span style="display: block;width: 5px;height: 5px;border-radius: 50%;background: #2eba07;margin-right: 8px;"></span><i style="font-style: normal;">待制作</i> 
				</div>
				<div style="display: flex;align-items: center;justify-content: center;" v-if="scope.row.state=='1'">
				<span style="display: block;width: 5px;height: 5px;border-radius: 50%;background: #108de9;margin-right: 8px;"></span><i style="font-style: normal;">制作中</i> 
				</div>
				<div style="display: flex;align-items: center;justify-content: center;" v-if="scope.row.state=='2'">
				<span style="display: block;width: 5px;height: 5px;border-radius: 50%;background: #999999;margin-right: 8px;"></span><i style="font-style: normal;">已上传</i> 
				</div>
...
import {  get , patch , post ,baseUrl } from '../api'
...
queryBtn(id){
this.$router.push({path:"/LibraryManagement",query:{fileID:id}})
},
btnUpload(id){
var arr = this.tableData
var _this = this;
